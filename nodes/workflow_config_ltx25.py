...
def _load_unet(name: str, gguf: bool = False):
    ...
    if not path:
        raise ValueError(f"[DAZ TOOLS] WorkflowConfigLtx25: diffusion model '{name}' not found")
    return comfy.sd.load_diffusion_model(path)


def _load_vae(name: str, kind: str = "VAE"):
    """Load a VAE from models/vae. Used for both the video and the audio VAE:
    LTX2.5's audio VAE ships with 'audio_vae.'/'vocoder.' key prefixes, which
    comfy.sd.VAE remaps to 'autoencoder.' on its own once it detects the LTX
    audio vocoder keys — so no separate audio loader is needed."""
    if not name:
        return None
    path = folder_paths.get_full_path("vae", name)
    if not path:
        raise ValueError(f"[DAZ TOOLS] WorkflowConfigLtx25: {kind} '{name}' not found")
...
def _load_clip(name: str):
    """LTX2.5 ships a single text encoder file that fuses the Gemma-4 12B encoder
    with the text_embedding_projection weights LTX2.3 kept in a second file, so
    one path is enough here."""
    if not name:
        return None
    path = folder_paths.get_full_path("text_encoders", name)
    if not path:
        raise ValueError(f"[DAZ TOOLS] WorkflowConfigLtx25: text encoder '{name}' not found")
...
def _load_image(path: str):
    ...
    if not os.path.exists(full):
        raise ValueError(f"[DAZ TOOLS] WorkflowConfigLtx25: image not found at '{full}'")
...
def _load_audio(path: str, start_s: float = 0.0, end_s: float = 0.0):
    ...
    if os.path.isabs(path):
        ...
        full = os.path.join(folder_paths.get_input_directory(), path)
    if not os.path.exists(full):
        raise ValueError(f"[DAZ TOOLS] WorkflowConfigLtx25: audio not found at '{full}'")
    return decode_audio_file(full, "WorkflowConfigLtx25", start_s=start_s, end_s=end_s)
...
def _load_lora(name: str):
    ...
    if not path:
        raise ValueError(f"[DAZ TOOLS] WorkflowConfigLtx25: lora '{name}' not found")
...
class WorkflowConfigLtx25:
    ...
    def load_config(self, movie: str, scene: str, take: str):
        ...
        if name is None:
            raise ValueError(
                f"[DAZ TOOLS] WorkflowConfigLtx25: '{scene}' not found"
            )
        entry      = configs[name]
        ...
        if _get_seed_randomize(seed_obj):
            ...
            try:
                ...
                print(f"[DAZ TOOLS] WorkflowConfigLtx25: could not save random seed — {e}")
        ...
        try:
            ...
            raise RuntimeError(f"[DAZ TOOLS] LTX2.5: transformer load failed ('{unet_name}'): {e}") from e
        try:
            video_vae = _load_vae(vae_name, "video VAE")
        except Exception as e:
            raise RuntimeError(f"[DAZ TOOLS] LTX2.5: video VAE load failed ('{vae_name}'): {e}") from e
        try:
        ...
        ref_audio = _load_audio(*resolve_audio_source(
            active_set, active_set.get("audio_path"), "WorkflowConfigLtx25: audio"))
        ...
        return (
            ...
            lora_7_sd, lora_8_sd,
            latent_upscaler,
            resolve_extended_media(active_set, "WorkflowConfigLTX25", ref_image, ref_audio),
        )
